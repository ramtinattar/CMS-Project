...
class AuthController extends Controller {
    ...
    public function registerAction($id = 'new') {
        ...
        if(!$user) {
            Session::msg("Tu n'as pas la permission de modifier cet utilisateur");
        ...
        if($this->request->isPost()){
            ...
            if($user->save()) {
                $msg = ($id == 'new')? "User ajoute." : "User actualisé";
                Session::msg($msg, 'success');
                Router::redirect('admin/users');
            }
            
        }
        $this->view->header = $id == 'new'? 'Ajouter Utilisateur' : 'Editer Utilisateur';
        $this->view->user = $user;
    ...
    public function loginAction(){
        ...
        if($this->request->isPost()) {
            ...
            if($isError) {
                $user->setError('email', 'Quelque chose ne va pas avec le e-mail ou le mot de passe. Veuillez réessayer.');
