...
class BlogController extends Controller {
    ...
    public function indexAction(){
        ...
        $this->view->heading = "Nouvelle Articles";
        $this->view->setSiteTitle('Nouvelle Articles');
        $this->view->render();
    }
    ...
    public function categoryAction($categoryId) {
        ...
        if(!$category) {
            Session::msg('Categorie non existant', 'warning');
            Router::redirect('');
        }
        $this->view->articles = Articles::find($params);
        $this->view->total = Articles::findTotal($params);
    ...
    public function authorAction($authorId) {
        ...
        if(!$author) {
            Session::msg("L’auteur n’existe pas.", 'warning');
            Router::redirect('');
        }
        $params = [
            'columns' => "articles.*, users.fname, users.lname, categories.name as category, categories.id as category_id",
    ...
    public function detailsAction($id) {
        ...
        if(empty($article->category_id)) {
            ...
            $article->category = "Non categorisé";
        }
