...
class AdminController extends Controller {
    ...
    public function articleAction($id = 'new') {
        ...
        if(!$article){
            Session::msg("Tu n’as pas la permission de modifier cet article");
            Router::redirect('admin/articles');
        }

        $categories = Categories::find(['order' => 'name']);
        $catOptions = [0 => 'Non categorisé'];
        ...
        $this->view->heading = $id === 'new'? "Ajouter Article" : "Editer Article";
        $this->view->render();
    }
    ...
    public function deleteArticleAction($id) {
        ...
        if($article) {
            Session::msg("Article Suprimme.", 'success');
            unlink(PROOT . DS . $article->img);
            $article->delete();
        } else {
            Session::msg("Tu n’as pas la permission de supprimer cet article.");
        }
        Router::redirect('admin/articles');
    }
    ...
    public function toggleBlockUserAction($userId) {
        ...
        if($user) {
            ...
            $user->save();
            $msg = $user->blocked? "Utilisateur bloqué." : "Utilisateur unbloqué.";
        }
    ...
    public function deleteUserAction($userId) {
        ...
        $user = Users::findById($userId);
        $msgType = 'danger';
        $msg = 'L’utilisateur ne peut pas être supprimé';
    ...
    public function categoryAction($id = 'new') {
        ...
        if(!$category){
            Session::msg("Category n'exist pas.");
        ...
        if($this->request->isPost()) {
            ...
            if($category->save()) {
                Session::msg('Category sauvegardé!', 'success');
                Router::redirect('admin/categories');
            }
        }

        $this->view->category = $category;
        $this->view->heading = $id == 'new'? "Ajouter Categorie" : "Editer Categorie";
    ...
    public function deleteCategoryAction($id) {
        ...
        if(!$category) {
            Session::msg("Le categorie n'exist pas");
            Router::redirect('admin/categories');
        } else {
            $category->delete();
            Session::msg("Category Suprimme.", 'success');
            Router::redirect('admin/categories');
        }
    }
